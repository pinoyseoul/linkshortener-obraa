...
Please check if `NUXT_SITE_TOKEN` is set to pure numbers, PinoySeoul does not support pure number Tokens, we consider this to be unsafe.
...
This is a feature of PinoySeoul. By default, we automatically convert all links to lowercase to avoid case-sensitive issues and improve usability. This ensures users don’t encounter errors due to accidental capitalization differences.
