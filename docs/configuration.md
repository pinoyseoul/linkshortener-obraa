# PinoySeoul Configuration

PinoySeoul provides some configuration options, which can be referred to in [.env.example](../.env.example).
...
The default PinoySeoul homepage is the introduction page, you can replace it with your own website.
...
2. Create R2 bucket: `wrangler r2 bucket create PinoySeoul`

Backups are stored in R2 with the path `backups/links-{timestamp}.json` and run daily at 00:00 UTC.

## `NUXT_SAFE_BROWSING_DOH`

Set to a DNS over HTTPS (DoH) endpoint URL to enable automatic unsafe link detection when creating or editing links. When enabled, PinoySeoul queries the DoH service to check if the destination domain is flagged as malicious. If the domain resolves to `0.0.0.0`, the link is automatically marked as unsafe and visitors will see a warning page before being redirected.
...
If this is not set, PinoySeoul will fall back to its default 404 page.
